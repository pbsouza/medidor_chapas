...
export const VisualCutDiagram: React.FC<Props> = ({
  ...
  // Estado de Tela Cheia (Fullscreen Modal / Mobile Landscape)
  ...
  const [fullscreenSheetRotation, setFullscreenSheetRotation] = useState<number>(0);
  const [showFullscreenPiecesDrawer, setShowFullscreenPiecesDrawer] = useState<boolean>(false);

  // Escuta tecla ESC para sair do modo tela cheia
  useEffect(() => {
  ...
  // Bloqueio de scroll de fundo durante tela cheia
  useEffect(() => {
    if (isFullscreen) {
  ...
  const handleEnterFullscreen = async () => {
    ...
    setFullscreenZoom(1);
    setFullscreenSheetRotation(0);

    // Detecta se a tela está em modo retrato em dispositivos móveis/portáteis
    const isPortrait = window.innerHeight > window.innerWidth;
    const isMobileOrTablet = window.innerWidth < 1024 || (typeof navigator !== 'undefined' && /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent));
    ...
    // Tenta API de Tela Cheia do navegador se disponível
    try {
      ...
      // Falha suave ignorada, pois o modal CSS fixed cobre toda a janela com 100vw e 100vh
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 mb-3 border-b border-slate-100">
        ...
        <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-bold rounded-lg flex items-center gap-1.5 transition-all shadow-xs cursor-pointer text-xs select-none"
            title="Expandir diagrama para tela cheia (otimizado para celular, tablet e computador)"
          >
      ...
      <div className="bg-slate-100/90 border border-slate-200 rounded-xl p-3 mb-3 flex flex-wrap items-center justify-between gap-2.5 text-xs">
        {/* Toggle de Ajuste Manual & Botão Tela Cheia Rápido */}
        <div className="flex items-center gap-2 flex-wrap">
          <button
            type="button"
            onClick={handleEnterFullscreen}
            className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold flex items-center gap-1.5 transition-all shadow-xs cursor-pointer active:scale-95"
            title="Abrir diagrama em tela inteira (computador, celular ou tablet)"
          >
            <Maximize2 className="w-3.5 h-3.5" />
          ...
          >
            ...
            <span className="whitespace-nowrap">{isManualEditMode ? '✓ Modo Ajuste Ativo' : 'Ajuste Manual (Arrastar)'}</span>
          </button>
      ...
      {/* ========================================================================= */}
      {/* MODAL DE TELA CHEIA (OTIMIZADO PARA COMPUTADOR, SMARTPHONE & TABLET)       */}
      {/* ========================================================================= */}
      {isFullscreen && (
        <div className="fixed inset-0 z-[99999] bg-slate-950/98 text-slate-100 flex flex-col overflow-hidden select-none animate-in fade-in duration-150">
          {/* BARRA SUPERIOR DE CONTROLES DO MODO TELA CHEIA */}
          <div className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-3 sm:px-5 py-2.5 flex items-center justify-between gap-2.5 z-20 shrink-0">
            {/* Informações da Chapa / Bobina */}
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <span
                className={`hidden xs:inline-flex items-center px-2 py-0.5 text-[10px] font-black rounded uppercase tracking-wider whitespace-nowrap shrink-0 ${
                  currentPlan.isScrap || currentPlan.stockCategory === 'retalho'
              ...
              >
                {currentPlan.isCoilCut ? '🌀 BOBINA' : '📋 CHAPA'}
              </span>

              <div className="min-w-0">
                <h3 className="font-black text-xs sm:text-sm text-white truncate flex items-center gap-1.5">
                  <span>{currentPlan.sheetName}</span>
                  <span className="font-mono text-slate-400 font-normal text-xs whitespace-nowrap">
                    ({currentPlan.width} × {currentPlan.length} mm)
                  </span>
                </h3>
                <div className="text-[11px] text-slate-400 flex items-center gap-2 font-medium truncate">
                  <span className="text-emerald-400 font-bold font-mono">
                    {currentPlan.yieldPercentage}% Rendimento
                  </span>
                  <span>• Desperdício: {GeometryService.formatAreaM2(currentPlan.wasteAreaMm2)}</span>
                  <span className="hidden sm:inline">• {currentPlan.placedPieces.length} peças</span>
                </div>
              </div>
            </div>

            {/* Ferramentas Rápidas no Modo Tela Cheia */}
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              {/* Controles de Zoom */}
              <div className="flex items-center bg-slate-800/90 rounded-lg p-1 border border-slate-700">
                <button
                  type="button"
                  onClick={() => setFullscreenZoom((z) => Math.max(0.5, z - 0.2))}
                ...
                <button
                  ...
                  className="px-2 py-0.5 text-[11px] font-mono font-bold text-slate-200 hover:text-white"
                  title="Zoom 100%"
                >
                ...
                <button
                  type="button"
                  onClick={() => setFullscreenZoom((z) => Math.min(3.0, z + 0.2))}
                  className="p-1 hover:bg-slate-700 text-slate-300 hover:text-white rounded transition-colors"
                  title="Aumentar Zoom"
                >
              ...
              {/* Botão de Alternar Modo Paisagem (Smartphone / Portátil Retrato) */}
              <button
                type="button"
                ...
                title="Girar diagrama para visualização horizontal / paisagem ampla em celulares e tablets"
              >
                <Smartphone className="w-4 h-4" />
                <span className="hidden md:inline">{forceLandscape ? 'Paisagem Ativa' : 'Girar Paisagem'}</span>
              </button>

              {/* Girar Chapa (90º em 90º) */}
              <button
                type="button"
                onClick={() => setFullscreenSheetRotation((r) => (r + 90) % 360)}
                className="px-2 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
                title="Girar orientação do diagrama em 90 graus"
              >
                <RotateCw className="w-4 h-4" />
                <span className="hidden lg:inline">{fullscreenSheetRotation}º</span>
              </button>

              {/* Drawer de Lista de Peças e Roteiro de Corte */}
              <button
                type="button"
                onClick={() => setShowFullscreenPiecesDrawer(!showFullscreenPiecesDrawer)}
                className={`px-2.5 py-1.5 rounded-lg border text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                  showFullscreenPiecesDrawer
                    ? 'bg-blue-600 text-white border-blue-500 shadow-blue-900/30'
                    : 'bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700'
                }`}
                title="Ver lista de peças e passos de corte da guilhotina"
              >
                <Layers className="w-4 h-4" />
                <span className="hidden sm:inline">Peças ({currentPlan.placedPieces.length})</span>
              </button>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-red-600 hover:bg-red-700 active:scale-95 text-white font-black text-xs sm:text-sm rounded-lg flex items-center gap-1.5 shadow-lg transition-all cursor-pointer ml-1"
                title="Sair do modo tela cheia (Tecla ESC)"
              >
                <X className="w-4 h-4 shrink-0" />
                <span>Sair</span>
                <span className="text-[10px] opacity-80 font-mono hidden sm:inline">(ESC)</span>
              </button>
            </div>
          </div>

          {/* ÁREA PRINCIPAL DO BLUEPRINT EM TELA CHEIA */}
          <div className="flex-1 relative w-full h-full flex items-center justify-center p-2 sm:p-6 overflow-auto bg-radial from-slate-900 to-slate-950">
            {/* Contêiner que suporta o Modo Paisagem Forçado e Zoom Dinâmico */}
            <div
              className="transition-all duration-300 origin-center flex flex-col items-center justify-center mx-auto"
              style={{
                width: forceLandscape
                  ? 'calc(100vh - 100px)'
                  : fullscreenZoom === 1
                  ? '100%'
                  : `${Math.max(700, Math.round(1200 * fullscreenZoom))}px`,
                maxWidth: forceLandscape ? 'none' : '100%',
                transform: `rotate(${forceLandscape ? 90 : fullscreenSheetRotation}deg)`,
                transformOrigin: 'center center',
              }}
            >
              {/* Eixos de Medidas do Topo */}
              <div className="w-full flex justify-between text-xs font-mono text-slate-400 font-bold uppercase pb-1.5 px-2">
                <span>0 mm</span>
                <span className="text-slate-300 font-bold tracking-wider">
                  Eixo Longitudinal: {currentPlan.length} mm ({(currentPlan.length / 1000).toFixed(2)}m) ➔
                </span>
                <span>{currentPlan.length} mm</span>
              </div>

              {/* Blueprint SVG em Alta Definição */}
              <div
                className="w-full relative border-2 border-slate-700 rounded-xl shadow-2xl overflow-hidden bg-white touch-none"
                style={{
              ...
              >
                <svg
                  ...
                  style={{
                    maxHeight: forceLandscape ? '80vw' : 'min(82vh, 900px)',
                    minHeight: '220px',
                  }}
                >
                  <defs>
                    ...
                    >
                      <line x1="0" y1="0" x2="0" y2="16" stroke="rgba(239, 68, 68, 0.4)" strokeWidth="2" />
                    </pattern>
                    <pattern
                      id={`fs-scrap-${index}`}
                      width="16"
                      ...
                      patternUnits="userSpaceOnUse"
                    >
                      <line x1="0" y1="0" x2="0" y2="16" stroke="rgba(245, 158, 11, 0.45)" strokeWidth="2" />
                    </pattern>
                  </defs>
                  ...
                  {currentPlan.remnants.map((r, rIdx) => {
                    ...
                    return (
                      <g key={`fs-rem-${r.id}-${rIdx}`}>
                        {r.polygonPoints ? (
                          <polygon
                            points={r.polygonPoints}
                            fill={r.isUsable ? `url(#fs-scrap-${index})` : `url(#fs-waste-${index})`}
                            stroke={r.isUsable ? '#d97706' : '#ef4444'}
                            strokeWidth="2"
                            strokeDasharray={r.isUsable ? '6,3' : '4,2'}
                          />
                  ...
                  {currentPlan.placedPieces.map((p, pIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <text
                            ...
                            y={geo.centroidY + 6}
                            fill={color.text}
                            fontSize={Math.max(13, Math.min(22, avgHeight / 3.2))}
                            fontWeight="bold"
                            textAnchor="middle"
                            className="font-sans select-none drop-shadow-sm uppercase pointer-events-none"
                          >
                    ...
                    return (
                      ...
                      >
                        ...
                        <text
                          x={p.x + p.length / 2}
                          y={p.y + p.devStart / 2 + 6}
                          fill={color.text}
                          fontSize={Math.max(13, Math.min(22, p.devStart / 3.2))}
                          fontWeight="bold"
                          textAnchor="middle"
                          className="font-sans select-none drop-shadow-sm uppercase pointer-events-none"
                        >
              ...
              {/* Eixos de Medidas da Base */}
              <div className="w-full flex justify-between text-xs font-mono text-slate-400 font-bold uppercase pt-1.5 px-2">
                <span>0 mm</span>
                <span className="text-slate-300">Largura da Chapa / Bobina: {currentPlan.width} mm</span>
                <span>{currentPlan.width} mm</span>
              </div>
            </div>

            {/* Painel Flutuante com Detalhes da Peça Selecionada */}
            {selectedPieceId && (
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-30 bg-slate-900/95 border border-blue-500/50 shadow-2xl rounded-xl p-3 text-xs flex items-center gap-3 backdrop-blur-md animate-in slide-in-from-bottom-2">
                {(() => {
                  const selPiece = currentPlan.placedPieces.find((p) => p.pieceId === selectedPieceId);
                  if (!selPiece) return null;
                  return (
                    <>
                      <span className="w-3 h-3 rounded-full bg-blue-500 shrink-0"></span>
                      <span className="font-bold text-white">
                        #{selPiece.cutIndex} {selPiece.pieceName}
                      </span>
                      <span className="text-slate-300 font-mono">
                        {selPiece.isTrapezoid
                          ? `${selPiece.devStart}→${selPiece.devEnd} mm × ${selPiece.length} mm`
                          : `${selPiece.devStart} × ${selPiece.length} mm`}
                      </span>
                      <span className="text-slate-400 text-[11px] font-mono">
                        (Posição: X={selPiece.x}mm, Y={selPiece.y}mm)
                      </span>
                      <button
                        type="button"
          ...
          {/* DRAWER LATERAL / INFERIOR DE PEÇAS & ROTEIRO DE CORTE (SE ABERTO) */}
          {showFullscreenPiecesDrawer && (
            <div className="bg-slate-900/95 border-t border-slate-800 p-3 sm:p-4 max-h-56 overflow-y-auto z-20 shrink-0 animate-in slide-in-from-bottom duration-200">
              <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-800">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
                  <Layers className="w-4 h-4 text-blue-400" />
                  <span>Roteiro de Corte & Peças da Chapa</span>
                </span>
                <button
                  type="button"
                  onClick={() => setShowFullscreenPiecesDrawer(false)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                ...
                <div className="space-y-1">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Peças nesta chapa:</div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                    {currentPlan.placedPieces.map((p, pIdx) => {
                      ...
                      return (
                        <div
                          key={`fs-drawer-p-${p.pieceId}-${pIdx}`}
                          onClick={() => setSelectedPieceId(isSelected ? null : p.pieceId)}
                        ...
                        >
                          <div className="flex items-center gap-1.5 truncate">
                            <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: color.stroke }}></span>
                            <span className="font-bold truncate">#{p.cutIndex} {p.pieceName}</span>
                          </div>
                          <span className="font-mono text-[10px] text-slate-400 shrink-0 ml-1">
                            {p.length}×{p.devStart}mm
                          </span>
                        </div>
                      );
                    })}
                ...
                {/* Sequência de Corte */}
                <div className="space-y-1">
                  <div className="text-[10px] font-bold uppercase text-slate-400">Passos de Guilhotina:</div>
                  <div className="space-y-1">
                    {currentPlan.cutSequence.slice(0, 4).map((step, sIdx) => (
                      <div
                        key={`fs-step-${sIdx}`}
                        className="flex items-start gap-2 p-1.5 rounded bg-slate-800/70 border border-slate-700 text-[11px]"
                      >
                        <span className="w-4 h-4 rounded-full bg-slate-700 text-white font-bold flex items-center justify-center text-[9px] shrink-0">
                          {step.step}
                        </span>
